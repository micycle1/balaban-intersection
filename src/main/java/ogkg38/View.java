...
public class View extends Component {
	...
	Point firstEditedPoint = null, viewRectFrom = new Point(0, 0), viewRectTo = new Point(100, 100), oldViewRectFrom,
			oldViewRectTo;
	...
	public View() {
		...
		addMouseWheelListener(new MouseWheelListener() {
			...
			public void mouseWheelMoved(MouseWheelEvent e) {
				...
				double x = getX(e.getX()), y = getY(e.getY());
				double dx = viewRectTo.x - viewRectFrom.x, dy = viewRectTo.y - viewRectFrom.y, cx = x - viewRectFrom.x,
						cy = y - viewRectFrom.y;
	...
	synchronized public void paint(Graphics g) {
		int cx1 = 0, cx2 = g.getClipBounds().width, cy1 = 0, cy2 = g.getClipBounds().height;
		if (editMode) {
		...
		for (Segment s : segments) {
			g.drawLine(getX(s.from.x), getY(s.from.y), getX(s.to.x), getY(s.to.y));
